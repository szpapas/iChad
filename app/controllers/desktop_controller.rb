...
class DesktopController < ApplicationController
  ...
  def generate_sg(archiveid, sm=false)
    ...
    if data.dalb.to_i == 3
      ...
      titles=split_string(tddj[0].tdzl) 
  ...
  def get_timage_from_db
    if (params['gid'].nil?)
      ...
      local_filename = './dady/'+user[0]["yxmc"].gsub('$', '_')
      small_filename = './dady/'+user[0]["yxmc"].gsub('$', '-')
      ...
      txt = "/assets/dady/#{user[0]["yxmc"].gsub('$', '-')}"
    end  
    render :text => txt
  end
  ...
  def export_image
    ...
    data = user[0]
    dh = "#{data.qzh}_#{data.mlh}_%_#{data.dalb}"
  ...
  def qbhd_jylc
    rq=Time.now.strftime("%Y-%m-%d")
    user = User.find_by_sql("update jylist set hdsj='#{rq}' where jyid = #{params['id']};")
