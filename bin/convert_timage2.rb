...
def convert_timage_table
  ...
  for kk in 0..total_image/BLOCK_SIZE
    ...
    vhs  = $conn.exec("select v_hash from yd_index order by v_hash offset #{BLOCK_SIZE*kk} limit #{BLOCK_SIZE});")
    vhs.each do |vv_hash|
      v_hash = vv_hash['v_hash']
      $conn.exec("insert into yd_#{kk}(v_hash) values ('#{v_hash}');")
      $conn.exec "update yd_#{kk} set data = ti.data from timage as ti where ti.v_hash = '#{v_hash}';" 
    end
