...
def save2timage(filename,dh,dh_prefix,czr,czrname)
  ...
  puts "delete from yun_timage where jyid = '#{dh}' and yxbh ='#{yxbh}';"  
  $conn.exec("delete from yun_timage where jyid = '#{dh}' and yxbh ='#{yxbh}';")
  #puts "insert file: #{path}  size: #{width}, #{height}  meta: #{meta_tz}   ... "

  puts "insert into yun_timage (rq,czr,czrname,width, height,dh, yxmc, yxbh, yxdx, meta, meta_tz, pixel,dh_prefix,sfzs) values ('#{rq}','#{czr}', '#{czrname}',#{width}', '#{height}','#{dh}', '#{yxmc}', '#{yxbh}', #{yxdx},  '#{meta}', #{meta_tz}, #{pixels}, '#{dh_prefix}',1);"
  #if meta!=''
  #  meta=meta.gsub('"','')
  #end
  $conn.exec("insert into yun_timage (rq,czr,czrname,width, height,jyid, yxmc, yxbh, yxdx, data, meta_tz, pixel,dh_prefix,sfzs) values ('#{rq}','#{czr}', '#{czrname}','#{width}', '#{height}','#{dh}', '#{yxmc}', '#{yxbh}', #{yxdx}, E'#{edata}' , #{meta_tz}, #{pixels}, '#{dh_prefix}',1);")
