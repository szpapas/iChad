...
$conn.exec("set standard_conforming_strings = off")
  ...
  pr_path="./dady"
  system("rm #{pr_path}/qztj.xls")
  ff = File.open(pr_path + "/qztj.xls" ,'w+')
  ff.write('<meta http-equiv="Content-Type" content="text/html; charset=UTF-8"><table cellpadding=8>')
  #list=$conn.exec("select * from d_dwdm  order by id;") 
  #distinct
  for j in 0..list.count-1
    puts list[j]['dwdm']
    ff.write('<tr><th style="text-align: center" colspan="15"><font size = 5>' + list[j]['dwdm'] + '</font></th></tr>'  )
    ...
    mlhcf=$conn.exec("select distinct cast(dalb as integer),mlh from archive where qzh='#{list[j]['id']}'  order by dalb,mlh;") 
